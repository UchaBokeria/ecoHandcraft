...
import { prisma, error, json } from '$lib/server/prisma';

export async function POST({ params }) {
	const { email } = params;

	const user = await prisma.users.findUnique({ where: { email } });
	if (!user) throw error(403, 'Bad parameter, email is wrong');
	...
	if (!reset) throw error(500, "something went wrong, user's confirmation status was not updated");
