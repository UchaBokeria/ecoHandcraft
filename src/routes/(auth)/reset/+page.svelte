<script>
    ...
    const reset = async () => {
        const user = await http.post(`api/auth/reset/${form.email.toLowerCase()}`)
            ...
            <div class="xl:w-full xl:max-w-sm 2xl:max-w-md xl:mx-auto">
                ...
                <form method="POST" class="mt-8">
                    <div class="space-y-5">
                        ...
                        <div>
                            <button on:click={reset} type="button" class="inline-flex items-center justify-center w-full px-4 py-4 text-base font-semibold text-white transition-all duration-200 bg-blue-600 border border-transparent rounded-md focus:outline-none hover:bg-blue-700 focus:bg-blue-700">Send email</button>
                        </div>
                    </div>
                </form>
            </div>
